...
mysql -uroot -D tushare -pAbcd1234 -e "SELECT CODE,INDUSTRY,round(1000/PRICE + 10 * GPR + 50 * BIPS + 10 * SHEQRATIO + 5 * CASHFLOWRATIO + 5 * if(MBRG>100,MBRG/10,MBRG) + 5 * if(EPSG>100,EPSG/10,EPSG) + 5 * if(SEG>100,SEG/10,SEG)) AS SCORE,round(PE),round(PRICE),round(GPR),round(BIPS),round(SHEQRATIO),CASHFLOWRATIO,MBRG,EPSG,SEG FROM RESULT_ALLSTOCKINDEX WHERE round(PE) > 0 and round(PE) < 70 AND CODE NOT LIKE '3%' AND CASHFLOWRATIO > 0 AND round(BIPS) > 0 AND MBRG > 0 AND EPSG > 0 AND SEG > 0 ORDER BY SCORE DESC, round(PE), round(PRICE) limit 0,50;" >Stock_Recommend2.txt
